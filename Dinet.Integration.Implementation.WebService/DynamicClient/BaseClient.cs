...
namespace Dinet.Integration.Implementation.WebService.DynamicClient
{
    ...
    public class BaseClient
    {
        ...
        /// <returns></returns>
        public static HttpWebRequest PostHeaderRequest(string url, bool autenticacion = false)
        {
            #region [Force TLS 2.0]
            ServicePointManager.Expect100Continue = true;
            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
            ...
            if (autenticacion)
            {
                ...
                var credentialsManager = CredentialManager.GetCredentials();

                string credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{credentialsManager.UserName}:{credentialsManager.Password}"));
                headerRequest.Headers.Add("Authorization", $"Basic {credentials}");
            }
